...
function FormulaDisclosure({ step }: { step: CalculationStep }) {
  ...
  return (
    <div className="formula-disclosure">
      <button className="text-button" type="button" aria-expanded={open} aria-controls={panelId} onClick={() => setOpen((current) => !current)}>
    ...
    <div className="results-stack" aria-live="polite">
      <section className="results-card" aria-labelledby="results-title">
        <div className="section-heading-row">
          ...
          <span className="result-status" role="status">Calcolo aggiornato</span>
        </div>
      ...
      <section className="card composition" aria-labelledby="composition-title">
        ...
        <div className="composition-bar" role="img" aria-label={segments.map((segment) => `${segment.label}: ${formatCurrency(segment.value)}, ${segment.percentage.toFixed(1)}%`).join('; ')}>
          {segments.map((segment) => <span key={segment.label} className={segment.className} style={{ width: `${segment.percentage}%` }} />)}
        </div>
        <ul className="legend">{segments.map((segment) => <li key={segment.label}><span className={`legend-dot ${segment.className}`} aria-hidden="true" /><span><strong>{segment.label}</strong><small>{formatCurrency(segment.value)} · {segment.percentage.toLocaleString('it-IT', { maximumFractionDigits: 1 })}%</small></span></li>)}</ul>
      </section>
...
function App() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(undefined)
    setResult(calculateSalary({ grossAnnualSalary: validation.amount, monthlyPayments: payments }))
  }
